class MaxHeap {
  ...
  pop() {
    ...
    if (this.#size > 1)
      //배열이 1보다 크면 - 자식 노드가 있으면
      this.#tree[1] = this.#tree.pop(); //마지막 자식 노드 pop해서 루트로 올려줌
    else this.#tree.pop(); //자식 노드 없으면 그대로 pop
    this.#size--;
  ...
  #shift_down(index) {
    ...
    let bigger_child =
      this.#tree[lchild] > this.#tree[rchild] ? lchild : rchild; //자식 노드 크기 비교

    if (this.#tree[index] < this.#tree[bigger_child]) {
      //루트 노드가 큰 자식 노드보다 작으면
